...
class Auth extends MX_Controller
{
	...
	function login()
	{
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			{
				// cara por quinta ou sexta pode ser? mais quase certo que vou
				...
				{						
					...
					//$this->load->view($this->dx_auth->login_view, $data);
	    			  ...
	    			  $this->template->write_view('conteudo',$this->dx_auth->login_view, $data); //posso carregar o conteúdo de uma view aqui
	    			  $this->template->render();
				}
			}
		}
	...
	function register()
	{
		...
		{	
			...
			if ($this->dx_auth->captcha_registration)
			{
				...
				$val->set_rules('recaptcha_response_field', 'Código de Confirmação', 'trim|xss_clean|required|callback_recaptcha_check');
			}
			...
			{	
				...
				{
					$data['auth_message'] = '
					<div class="alert alert-success"><b>Usuário criado com sucesso! Cheque seu e-mail para ativar sua conta.</b></div>';
				}
				else
				{					
				...
				# Página de sucesso na criação
				$this->load->view($this->dx_auth->register_success_view, $data);

			}
			else
			{
				# Carrega página de Registro
				//$this->load->view('auth/register_form');
				$this->template->set_template('adm');
				$this->template->write_view('conteudo','auth/register_form'); //posso carregar o conteúdo de uma view aqui
				$this->template->render();
			}
		}
		elseif ( ! $this->dx_auth->allow_registration)
		{
			$data['auth_message'] = 'Registros de novos usuários está desabilitado.';
			$this->load->view($this->dx_auth->register_disabled_view, $data);
		}
		else
		{
			$data['auth_message'] = '<div class="alert alert-error"><b>ERRO: Será necessário primeiramente logar ao sistema.</b> <a href="'.site_url().'auth/logout" class="alert alert-error">Deseja logar no sistema?</a></div>';
			$this->load->view($this->dx_auth->logged_in_view, $data);
		}
	}
